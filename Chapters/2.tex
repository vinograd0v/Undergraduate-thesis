...
Como se mencionó anteriormente, probaremos el TNP usando una equivalencia que demostraremos en esta sección, para eso, primero vamos a definir usando como referencia el libro de Apostol \cite{Apostol:105425} la integral de Riemann Stieltjes
...
\begin{note}
    Observe que cuando un número $A$ existe, es único y se denota por $$\int_a^b fd\alpha \quad \text{o} \quad \int_{a}^{b} f(x)d\alpha(x).$$ 
La función $f$ es llamada \textit{integrando} y la función $\alpha$ es llamada \textit{integrador}. En el caso de que exista $A$ diremos que existe \textit{la integral de Riemann-Stieltjes} de $f$ con respecto a $\alpha$ en $[a,b]$. 
\end{note}


Note que la integral de Riemann no es más que un caso particular, cuando $\alpha(x)=x$, la utilidad de esta integral en teoría analítica de números se debe a que cualquier suma finita se puede escribir como una integral de Riemann-Stieltjes.\\

\begin{theorem}
  Sea $\alpha$ una función escalonada definida en $[a, b]$ con salto $\alpha_k$ en $x_k$.\\
  
Sea $f$ una función definida en $[a, b]$ tal que $f$ y $\alpha$ no sean ambas discontinuas a la derecha o a la izquierda de cada $x_k$. Entonces $\int_a^b f d \alpha$ existe y se tiene
$$
\int_a^b f(x) d \alpha(x)=\sum_{k=1}^n f\left(x_k\right) \alpha_k
$$
\end{theorem}

La prueba de esto nuevamente se encuentra en \cite{Apostol:105425}, allí también se encuentran algunas de las propiedades de esta integral que usaremos en esta sección, estas son el "Teorema de reducción a una integral de Riemann" \text{ y } la "\text{}Integración por partes" que se presentarán a continuación.\\
...
Comencemos por la fórmula de integración por partes, tenemos que esta se enuncia de la siguiente manera:
\begin{theorem}
...
\begin{proof}
    Sea $\varepsilon>0$ arbitrario. Como, tenemos por hipótesis que $\int_{a}^{b}fd\alpha$ existe, entonces existe una $P_{\varepsilon}\in \mathcal{P}[a,b]$ tal que para toda $P'\supset P_\varepsilon$, por lo cual tenemos que $$\biggm|S(P',f,\varepsilon) - \int_{a}^{b}fd\alpha\biggm|<\varepsilon.$$
    
    Tomando la suma de Riemann-Stieltjes de $\alpha$ con respecto a $f$ en $[a,b]$ dada por $S(P,\alpha,f)$ donde $P\supset P_\varepsilon$. Se tiene que,
    \begin{align*}
        S(P,\alpha,f)
        &= \sum_{k=1}^{n}\alpha(t_k)\Delta f_k
        \\
        &= \sum_{k=1}^{n}\alpha(t_k)f(x_k) - \sum_{k=1}^{n}\alpha(t_k)f(x_{k-1}). 
    \end{align*}
    Adicionalmente se tiene que:  
    \begin{align*}
        f(b)\alpha(b)-f(a)\alpha(a)
        &= \sum_{k=1}^{n}f(x_k)\alpha(x_k)-f(x_{k-1})\alpha(x_{k-1})
        \\
        &= \sum_{k=1}^{n}\Delta (f\alpha)_k
        \\
        &= \sum_{k=1}^{n}f(x_k)\alpha(x_k)-\sum_{k=1}^{n}f(x_{k-1})\alpha(x_{k-1})
    \end{align*}
    Por lo cual 
    \begin{align*}
        f(b)\alpha(b)-f(a)\alpha(a)-S(P,\alpha,f)&=\sum_{k=1}^{n}f(x_k)\alpha(x_k)-\sum_{k=1}^{n}f(x_{k-1})\alpha(x_{k-1})\\&-\sum_{k=1}^{n}\alpha(t_k)f(x_k) + \sum_{k=1}^{n}\alpha(t_k)f(x_{k-1})\\
        \text{Para $\quad P'=P\cup\{t_k | k=1,2,\cdots ,n\}$}&=\sum_{k=1}^{n}f(x_k)[\alpha(x_k)-\alpha(t_k)]\\
        &+ \sum_{k=1}^{n}f(x_{k-1})[\alpha(t_{k})-\alpha(x_{k-1})]\\
        &=\sum_{k=1}^{n}f(x_k)[\alpha(x_k)-\alpha(t_k)] + \sum_{k=1}^{n}f(x_{k-1})[\alpha(t_{k})-\alpha(x_{k-1})]\\
        &=S(P',f,\alpha)
    \end{align*}

    Por lo cual podemos decir que 
    $$\biggm|f(b)\alpha(b)-f(a)\alpha(a)-S(P,\alpha,f)-\int_{a}^{b}fd\alpha\biggm|<\varepsilon$$
    Esto quiere decir que, $\int_{a}^{b}\alpha df$ existe y luego como$$\biggm|S(P,\alpha,f)+\int_{a}^{b}fd\alpha +f(a)\alpha(a)-f(b)\alpha(b)\biggm|<\varepsilon$$
    Entonces $$\int_{a}^{b}\alpha df=f(b)\alpha(b)-f(a)\alpha(a)-\int_{a}^{b}fd\alpha.$$
\end{proof}
Con esto quedó demostrada la integración por partes en una Integral de Riemann-Stieljes.\\
Ahora vamos a enunciar el teorema de ``reducción a Riemann'', la demostración de este hecho se puede encontrar en \cite{Apostol:105425}.
\begin{theorem}
    Sea $ f \in R(\alpha) $ en $[a, b]$, donde $ \alpha \in \mathbb{C}^1[a, b]$, entonces,
